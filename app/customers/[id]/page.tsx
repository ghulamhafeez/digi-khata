...
export default function CustomerDetailPage() {
  ...
  const filtJama  = filtered.filter((t) => t.type === "jama").reduce((s, t) => s + t.amount, 0);
  const filtWapsi = filtered.filter((t) => t.type === "wapsi").reduce((s, t) => s + t.amount, 0);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
            ...
            <Dialog open={addOpen} onOpenChange={setAddOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>Jama (diya) ya Wapsi (liya) — {customer.name}</DialogDescription>
                </DialogHeader>
                <AddTransactionForm
                  customers={[customer]}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-5">
              ...
              <p className="text-2xl font-bold text-red-600 mt-1">Rs. {totalJama.toLocaleString()}</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-5">
              ...
              <p className="text-2xl font-bold text-green-600 mt-1">Rs. {totalWapsi.toLocaleString()}</p>
              <p className="text-xs text-gray-400 mt-0.5">Cash received</p>
            </div>

            <div className={`rounded-2xl border shadow-lg p-5 ${netUdhaar > 0 ? "bg-orange-50 border-orange-200" : "bg-white border-gray-100"}`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${netUdhaar > 0 ? "bg-orange-100" : "bg-green-100"}`}>
                <Receipt className={`h-5 w-5 ${netUdhaar > 0 ? "text-orange-500" : "text-green-500"}`} />
              </div>
              <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Net Udhaar</p>
              <p className={`text-2xl font-bold mt-1 ${netUdhaar > 0 ? "text-orange-600" : "text-green-600"}`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden">
          ...
          {transactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
              ...
              <div>
                <p className="font-medium text-gray-600">Koi transaction nahi</p>
                <p className="text-sm text-gray-400 mt-1">Pehla Jama ya Wapsi record karein.</p>
              </div>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          ...
          <EditCustomerForm
            customer={customer}
            onSuccess={() => { setEditOpen(false); fetchAll(); }}
        ...
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              {confirmTx && (
                <>
                  Permanently delete this <strong>{confirmTx.type === "jama" ? "Jama" : "Wapsi"}</strong> of{" "}
                  <strong>Rs. {confirmTx.amount.toLocaleString()}</strong>
                  {confirmTx.description ? ` (${confirmTx.description})` : ""}? This cannot be undone.
                </>
              )}
            </DialogDescription>
          </DialogHeader>
          <div className="flex gap-2 justify-end pt-2">
            <Button variant="outline" onClick={() => setConfirmTx(null)}>Cancel</Button>
            <Button
              variant="destructive"
              disabled={!!deletingId}
